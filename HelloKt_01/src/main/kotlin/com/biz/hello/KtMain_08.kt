...
fun main(){
    ...
    var xVer : Any = "Republic of Korea"
    var x = when(xVer) {
        is String -> xVer.indexOf("Korea") > 0 // 문자열이 포함되어 있냐?
        is String -> xVer.startsWith("of") // 시작하는 문자열이 of  이냐?
        else -> false
    }
    println("$xVer 에는 Korea 문자열이 ${if(x) "포함 되었음" else "포함되지 않음"}")
